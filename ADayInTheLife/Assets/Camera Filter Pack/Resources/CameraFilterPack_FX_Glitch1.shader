...
float4 frag (v2f i) : COLOR
{
	...
	float change_rnd = rand( trunc(uv.yy,float2(16)) + 150.0 * ct);
	float tf = 16.0*change_rnd;
	float t = 5.0 * trunc( _TimeX_s, tf);
	float vt_rnd = 0.5 * rand( trunc(uv.yy + t, float2(11)));
	vt_rnd += 0.5 * rand(trunc(uv.yy + t, float2(7)));
	vt_rnd = vt_rnd * 2.0 - 1.0;
	vt_rnd = sign(vt_rnd) * sat( ( abs(vt_rnd) -0.6) / (0.4) );
	float2 uv_nm = uv;
	uv_nm = sat( uv_nm + float2(0.1*vt_rnd, 0));
	float rnd = rand( float2( trunc( _TimeX_s, 8.0 )));
