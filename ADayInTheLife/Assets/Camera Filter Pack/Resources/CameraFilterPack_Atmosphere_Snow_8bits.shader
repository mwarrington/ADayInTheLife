...
float4 frag (v2f i) : COLOR
{
   ...
   float ts=uv.y-.2+sin(uv.x*4.0+7.4*cos(uv.x*10.0))*0.005;
    ...
    col=lerp(col,float3(1.0),smoothstep(0.9,1.0, s * .9 * _Value));
    
 	s=rand(mod(uv * 1.07 +float2(_TimeX,_TimeX)*float2(0.02,0.501),1.0)).r;
 	col=lerp(col,float3(1.0),smoothstep(0.9,1.0, s * 1. * _Value));
    
	s=rand(mod(uv+float2(_TimeX,_TimeX)*float2(0.05,0.5),1.0)).r;
    col=lerp(col,float3(1.0),smoothstep(0.9,1.0, s * .98 * _Value));
	
	s=rand(mod(uv * .9 +float2(_TimeX,_TimeX)*float2(0.02,0.51),1.0)).r;
    col=lerp(col,float3(1.0),smoothstep(0.9,1.0, s * .99 * _Value));
    ...
    col=lerp(col,float3(1.0),smoothstep(0.94,1.0, s * 1. * _Value));
	
	s=rand(mod(uv * .3 +float2(_TimeX,_TimeX)*float2(0.02,0.497),1.0)).r;
    col=lerp(col,float3(1.0),smoothstep(0.95,1.0, s * 1. * _Value));
	
	s=rand(mod(uv * .1 +float2(_TimeX,_TimeX)*float2(0.0,0.51),1.0)).r;
	col=lerp(col,float3(1.0),smoothstep(0.96,1.0, s * 1. * _Value));
	
	s=rand(mod(uv * .03 +float2(_TimeX,_TimeX)*float2(0.0,0.523),1.0)).r;
	col=lerp(col,float3(1.0),smoothstep(0.99,1.0, s * 1. * _Value));
