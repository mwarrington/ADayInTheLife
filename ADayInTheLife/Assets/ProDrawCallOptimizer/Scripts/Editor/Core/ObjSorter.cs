namespace ProDrawCall {
	...
	public static class ObjSorter {
	    ...
	    public static void Initialize() {
            ...
            optimizableShaders.Add(new OptimizableShader(""));
	    }
	        ...
	        for(int i = 0; i < objs.Count; i++) {
	            for(int j = 0; j < objs[i].Objects.Count; j++) {
	                ...
	                if(index == -1) {//no shader found for this object so add a new position
                        optimizableShaders.Add(new OptimizableShader(shaderName));
                        optimizableShaders[optimizableShaders.Count-1].AddObject(objs[i].Objects[j]);
	                } else {//shader already exists so place it in the sorted objs
                        optimizableShaders[index].AddObject(objs[i].Objects[j]);
	                }
	            }
	        }
	    ...
	    public static void AdjustArraysSize(int newSize) {
	        ...
	        if(offset > 0) {
	            for(int i = 0; i < offset; i++) {
                    optimizableShaders[optimizableShaders.Count-1].AddObject(null);
	            }
	        } else {
	            offset *= -1;
	            for(int i = 0; i < offset; i++) {
	    ...
	    public static int GetAproxAtlasSize(int index, bool reuseTextures) {
            return optimizableShaders[index].CalculateAproxAtlasSize(reuseTextures);
	    }
